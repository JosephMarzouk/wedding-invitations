...
export default function Gallery({ photos, alt }: { photos: string[]; alt: string }) {
  ...
  return (
    <section style={{ position: "relative", padding: "40px 16px 80px", background: "radial-gradient(ellipse 60% 40% at 20% 30%, color-mix(in srgb, var(--primary) 28%, transparent), transparent 70%), radial-gradient(ellipse 50% 35% at 80% 60%, color-mix(in srgb, var(--candle) 12%, transparent), transparent 70%), linear-gradient(180deg, var(--bg) 0%, color-mix(in srgb, var(--primary) 12%, var(--bg)) 50%, var(--bg) 100%)" }}>
          ...
          return (
            <div key={src + i} style={{ position: "absolute", left: pos[k].l, top: `calc(${row * base}px + ${pos[k].t})`, width: pos[k].w, animation: `floaty ${DUR[k]}s ease-in-out ${DELAY[k]}s infinite`, zIndex: c.active ? 10 : i + 1 }}>
              <div
                ...
                style={{
                  background: "color-mix(in srgb, var(--candle) 30%, white)", padding: "8px 8px 16px", border: "1px solid var(--accent)",
