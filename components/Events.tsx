...
function VenueCard({ ev, image, locale, city, s }: { ev: Ev; image: string; locale: string; city: string; s: Strings }) {
  ...
  return (
    <div style={{ width: "min(520px,100%)", margin: "0 auto", display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={{ position: "relative", height: "min(78vh,620px)", borderRadius: 28, overflow: "hidden", boxShadow: "0 30px 60px -20px rgba(0,0,0,.7)" }}>
        ...
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg, color-mix(in srgb, var(--bg) 5%, transparent) 30%, color-mix(in srgb, var(--bg) 55%, transparent) 65%, color-mix(in srgb, var(--bg) 96%, transparent) 100%)" }} />
        <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, padding: "0 28px 32px", display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
          ...
          <h2 style={{ margin: 0, fontWeight: 400, fontSize: "clamp(26px,4vw,34px)", lineHeight: 1.15, color: "var(--ink)", textWrap: "balance" }}>{main}</h2>
          {secondary && (
            <div lang={ar ? "en" : "ar"} dir={ar ? "ltr" : "rtl"} style={{ fontFamily: ar ? "var(--font-latin), serif" : "var(--font-arabic), serif", fontSize: "clamp(20px,3vw,26px)", lineHeight: 1.5, color: "var(--candle)" }}>{secondary}</div>
          )}
          <div style={{ marginTop: 6, fontSize: 18, color: "var(--soft)", fontVariantNumeric: "oldstyle-nums" }}>
            {formatClock(ev.start, locale)} – {formatClock(ev.end, locale)}
          </div>
        </div>
      </div>
      <div className="w-glass" style={{ padding: 22, display: "flex", flexDirection: "column", gap: 18, backdropFilter: "none", background: "color-mix(in srgb, var(--candle) 6%, transparent)", borderColor: "color-mix(in srgb, var(--accent) 30%, transparent)" }}>
...
export default function Events({ events, images, locale, city, strings }: Props) {
  ...
  return (
    <section style={{ position: "relative", padding: "40px 16px 110px", background: "linear-gradient(180deg, var(--bg), color-mix(in srgb, var(--primary) 10%, var(--bg)) 50%, var(--bg))", display: "flex", flexDirection: "column", gap: 40 }}>
      {events.map((ev, i) => (
